...
function preload() {
    ...
    if (pageIndex > 0 && pageIndex < $("body>section").size())
        selector = $("body>section").eq(pageIndex);
...
function pageLoad() {
    ...
    for (var i = 0; i < $(".video .swiper-slide").size(); i++)
    ...
    $(window).bind("resize", function(e) {
        ...
        if (pageIndex == 0)
            selector = ".video .swiper-slide.active .box";
...
function initLayout() {
    ...
    $(".video .swiper-slide").each(function(i, item) {
        if ($(item).hasClass("nth1")) {
            ...
            boxHeight = 207;
            if ($(item).find(".box .bottom").css("font-size") == "12px")
                boxHeight = 134;
    ...
    box = $(".quality .box");
    boxTop = parseInt(($(window).height() - box.height()) / 2 + hhFixed);
    box.css("top", boxTop);
    ...
    if ($(window).width() > 640){
        ...
        itemsTop = parseInt(($(window).height() - items.height() + hhFixed - $(".aboutus .menu").height()) / 2 + 120);
    } else{
        itemsTop = parseInt(($(window).height() - items.height() + hhFixed - $(".aboutus .menu").height()) / 2 + 100);
    }
...
function videoSwiperAnimation() {
    ...
    if (videoSwiper.activeLoopIndex == 1) {
        if (e.find(".box").width() == $(window).width())
